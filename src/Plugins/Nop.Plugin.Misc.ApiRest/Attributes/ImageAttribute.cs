...
using Nop.Plugin.Misc.ApiRest.DTO.Images;
...
namespace Nop.Plugin.Misc.ApiRest.Attributes
{
    public class ImageValidationAttribute : BaseValidationAttribute
    {
        ...
        public override async Task ValidateAsync(object instance)
        {
            ...
            if (imageSrcSet || imageAttachmentSet)
            {
                ...
                if (_errors.Count == 0)
                {
                    ...
                    await ValidatePictureByteArrayAsync(imageBytes, mimeType, Path.GetFileName(imageDto.Src));
                }
        ...
        private async Task ValidatePictureByteArrayAsync(byte[] imageBytes, string mimeType, string fileName)
        {
            if (imageBytes != null)
            {
                try
                {
                    imageBytes = await _pictureService.ValidatePictureAsync(imageBytes, mimeType, fileName);
                }
                catch (Exception ex)
                {
