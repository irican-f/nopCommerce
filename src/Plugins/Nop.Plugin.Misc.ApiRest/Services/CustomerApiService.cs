...
namespace Nop.Plugin.Misc.ApiRest.Services
{
    ...
    {
        private static readonly string FIRST_NAME = nameof(Customer.FirstName);
        private static readonly string LAST_NAME = nameof(Customer.LastName);
        private static readonly string LANGUAGE_ID = nameof(Customer.LanguageId);
        private static readonly string CURRENCY_ID = nameof(Customer.CurrencyId);
        private static readonly string DATE_OF_BIRTH = nameof(Customer.DateOfBirth);
        private static readonly string GENDER = nameof(Customer.Gender);
        private static readonly string VAT_NUMBER = nameof(Customer.VatNumber);
        private static readonly string VAT_NUMBER_STATUS_ID = nameof(Customer.VatNumberStatusId);
        private static readonly string EU_COOKIE_LAW_ACCEPTED = "EuCookieLaw.Accepted";
        private static readonly string COMPANY = nameof(Customer.Company);
		...
		{
            //var store = await _storeContext.GetCurrentStoreAsync();
            await _genericAttributeService.SaveAttributeAsync(customer, LANGUAGE_ID, language?.Id ?? 0/*, store.Id*/);
        }
		...
		public async Task<Currency> GetCustomerCurrencyAsync(Customer customer)
		{
            ...
            var customerCurrencyId = await _genericAttributeService.GetAttributeAsync<int>(customer, CURRENCY_ID/*, store.Id*/);
		...
		{
            //var store = await _storeContext.GetCurrentStoreAsync();
            await _genericAttributeService.SaveAttributeAsync(customer, CURRENCY_ID, currency?.Id ?? 0/*, store.Id*/);
        }
	}
}